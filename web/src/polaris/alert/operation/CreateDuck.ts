...
const validator = CreateForm.combineValidators<Values, {}>({
  ...
  monitor_type(v) {
    if (!v) return i18n.t('请填写监控类型')
  },
  alter_expr(v, meta) {
    const res = CreateForm.combineValidators<AlterExpr>({
      metrics_name(v) {
        if (!v) return i18n.t('请选择请求指标')
      },
      expr(v) {
        if (!v) return i18n.t('请选择表达式')
      },
      value(v) {
        if (!v) return i18n.t('请选择阈值')
      },
      for(v) {
        if (!v) return i18n.t('请输入持续时长')
      },
      for_unit(v) {
        if (!v) return i18n.t('请选择持续时长单位')
      },
    })(v, meta)
    return res
  },
  interval(v) {
    if (!v) return i18n.t('请选择告警周期')
  },
  interval_unit(v) {
    if (!v) return i18n.t('请选择告警周期')
  },
  topic(v) {
    if (!v) return i18n.t('请输入告警主题')
  },
  message(v) {
    if (!v) return i18n.t('请输入告警消息')
  },
})
