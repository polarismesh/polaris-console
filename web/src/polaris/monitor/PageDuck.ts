...
import { save } from "../common/components/csv";
...
export default class MonitorDuck extends Page {
  ...
  get reducers() {
    ...
    return {
      ...
      metricQuerySets: reduceFromPayload(
        types.SET_METRIC_QUERY_SETS,
        [] as MetricQuerySet[]
      ),
  ...
  get creators() {
    ...
    return {
      ...
      setMetricQuerySets: createToPayload<MetricQuerySet[]>(
        types.SET_METRIC_QUERY_SETS
      ),
      ...
      changeFilterConfig: createToPayload<FilterConfig>(
        types.CHANGE_FILTER_CONFIG
      ),
  ...
  *loadAllLabelSelect(needLoad: boolean = false) {
    const { types, creators, ducks, selectors, metricNames } = this;
    ...
    for (let index in this.monitorLabels) {
  ...
  *searchPreCheck() {
    ...
    console.log(labelList, queryNumber);
  ...
  *saga() {
    ...
    yield takeLatest(types.ADD_GRAPH, function* () {
      ...
      const queryFilterString = monitorFilters
        .map((item) => `${item.labelKey}="${item.labelValue}"`)
        .join(",");
      const query = `${metricName}{${queryFilterString}}`;
      const fetcherId = Math.floor(Math.random() * 1000).toString();
      const fetcher = yield* getFetcher(
    ...
    yield takeLatest(types.FETCH_LABELS, function* (action) {
      ...
      const match = metricNames.map(
        (metricName) => `${metricName}{${labelConstraint.join(",")}}`
      );
      yield fetcher.fetch({
    ...
    yield takeLatest(types.MODIFY_GRAPH, function* (action) {
      ...
      const query = `${metricName}{${queryFilterString}}`;
      const fetcher = yield* getFetcher(
        `${encodeURIComponent(query)}-${querySet.fetcherId}`
      );
      yield fetcher.fetch({ query, start, end, step });
      const prevQuerySetIndex = metricQuerySets.findIndex(
        (item) => item.fetcherId === querySet.fetcherId
      );
    ...
    yield takeLatest(types.CHANGE_FILTER_CONFIG, function* (action) {
      const { changedItem, filterConfig } = action.payload;
      yield put({ type: types.SET_FILTER_CONFIG, payload: filterConfig });
    });
    yield takeLatest(types.SEARCH, function* () {
      ...
      const step =
          ...
          : 3600;
      let metricQuerySets = [];
      let continueSearch = yield duck.searchPreCheck();
      if (!continueSearch) return;
      //求所有筛选条件以及指标的笛卡尔积
      metricNames.forEach((metricName) => {
        const labelList = Object.keys(filterLabels);
        const filteredLabelList =
          labelList.filter((labelKey) => filterLabels[labelKey].length !== 0) ||
          [];
      ...
      for (let index in metricQuerySets) {
        const metricQuerySet = metricQuerySets[index];
        const fetcher = yield* getFetcher(
          `${encodeURIComponent(metricQuerySet.query)}-${
            metricQuerySet.fetcherId
          }`
        );
    ...
    yield takeLatest(types.SAVE_CONFIG, function* (action) {
    ...
    yield takeLatest(types.GET_MONITOR_CONFIG, function* (action) {
      const savedConfigString = window.localStorage.getItem(
        `${duck.type}MonitorConfigLocalStorageKey`
      );
      yield put({
        type: types.SET_FILTER_CONFIG,
