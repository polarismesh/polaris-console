...
import { Form, FormItem, Button } from 'tea-component'
...
export default function Create(props: DuckCmpProps<Duck>) {
  ...
  return (
    <Dialog duck={duck} store={store} dispatch={dispatch} size='l' title={t('新建监控图表')}>
      ...
      <Form>
        <FormItem label={t('时间选择')}>
          <TimeSelect
            ...
            from={isModify ? new Date(start.getValue() * 1000).toString() : undefined}
            to={isModify ? new Date(end.getValue() * 1000).toString() : undefined}
          />
        </FormItem>
        <FormField field={metricName} label={t('指标名')} required>
          <Select type={'simulate'} appearance={'button'} field={metricName} options={MetricNameOptions} size='m' />
        </FormField>
        <FormItem label={t('筛选条件')}>
          <Form>
            {[...monitorFilters.asArray()].map((field, index) => {
              ...
              return (
                <Form layout={'inline'}>
                  ...
                  <FormField showStatusIcon={false} field={labelValue} label={t('条件值')}>
                  ...
                  {monitorFilters.getValue().length > 1 && (
                    <FormItem>
                      <Button
                        type='icon'
                        icon='close'
                        onClick={() => removeArrayFieldValue(monitorFilters, index)}
                      ></Button>
                    </FormItem>
                  )}
                </Form>
            ...
            <Button type={'icon'} icon={'plus'} onClick={() => addFilter(monitorFilters)}></Button>
          </Form>
        </FormItem>
      </Form>
    </div>
