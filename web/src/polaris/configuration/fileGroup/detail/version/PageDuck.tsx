...
export default class ConfigFileReleaseHistoryDuck extends GridPageDuck {
  ...
  *saga() {
    ...
    yield takeLatest(types.ROLLBACK, function*(action) {
      ...
      if (confirm) {
        const result = yield RollbackConfigFileReleases({
          rollbackConfigFileReleases: [{ ...action.payload, uniId: undefined }],
        })
        if (result) {
          notification.success({ description: '正在回滚中' })
          yield put(creators.reload())
        } else {
          notification.error({ description: '回滚提交失败' })
        }
      }
    })
