  ...
  *onSubmit() {
    ...
    if (options?.isModify) {
      ...
      if (options?.isModifyPassword) {
        ...
        if (result) {
          notification.success({ description: t('修改密码成功') })
          if (id === getUin()) {
            notification.warning({ description: t('您已成功重置密码，请重新登录。') })
            yield delay(3000)
            userLogout()
          }
        } else {
          notification.error({ description: t('修改密码失败') })
        }
      } else {
...
const validator = CreateForm.combineValidators<Values, DialogOptions>({
  name(v, values, meta) {
    ...
    if (!v.match(/^[\u4E00-\u9FA5A-Za-z0-9_\\-]+$/)) {
      return t('只能使用中文、数字、大小写字母 以及- _组成')
    }
    if (v.length > 64) {
      return t('最大长度为64')
    }
