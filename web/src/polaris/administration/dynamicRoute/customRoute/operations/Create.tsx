...
export default purify(function CustomRoutePage(props: DuckCmpProps<CreateDuck>) {
  ...
  function getArgumentsKeyComp(
    ...
    const labelList = [
      ...(keyField.getValue()
        ? [{ text: t('(输入值){{attr0}}', { attr0: keyField.getValue() }), value: keyField.getValue() }]
        : []),
      ...filteredLabelList,
    ]
    let keyComponent
    if (labelType.getValue() === RoutingArgumentsType.CUSTOM || type === 'destination') {
      ...
      keyComponent = (
        <Input placeholder={t('请输入Key值')} field={keyField} onChange={(key) => keyField.setValue(key)} />
      )
    }
    return (
  ...
  function getArgumentsValueComp(recordField: FieldAPI<RouteSourceArgument | RouteDestinationArgument>, type: string) {
    ...
    const options = [
      ...(valueField.getValue()
        ? [{ text: t('(输入值){{attr0}}', { attr0: valueField.getValue() }), value: valueField.getValue() }]
        : []),
      ...valueOptions,
    ]
    let valueComponent
                              ...
                              options={[
                                ...new Set([
                                  ...
                                  ...(sourceService.getValue()
                                    ? [
                                        {
                                          text: t('(输入值){{attr0}}', { attr0: sourceService.getValue() }),
