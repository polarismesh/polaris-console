...
export default purify(function AccessLimitingDetailPag(props: DuckCmpProps<AccessLimitingDetailPageDuck>) {
  ...
  const editTitle = composedId?.id && isReadOnlyModel ? '查看' : '编辑';
  ...
  return (
    <DetailPage
      ...
      duck={duck}
      dispatch={dispatch}
      title={composedId?.id ? `${editTitle}服务限流规则` : '新建服务限流规则'}
      backRoute={backRoute}
    >
      <Card>
        <Card.Body>
          <Form>
            <FormField label='限流规则名称' field={nameField} message='最长64个字符' required>
              <Input field={nameField} maxLength={64} size='l' readonly={isReadOnlyModel} />
            </FormField>
                  ...
                  <Form style={{ padding: '20px' }}>
                    <FormField field={namespaceField} label='命名空间' required>
                      <Select
                        ...
                        placeholder='请选择命名空间'
                        size='m'
                        disabled={!!composedId?.namespace || isReadOnlyModel}
                      />
                    </FormField>
                      ...
                      >
                        {ref => (
                          <TeaInput
                            ...
                            disabled={!!composedId?.service || isReadOnlyModel}
                          />
                        )}
                      </AutoComplete>
                    </FormField>
                    ...
                    <Form.Item label='请求匹配规则' align='middle' tips={MatchingLabelTips}>
                      ...
                      <div style={{ marginTop: '8px', display: `${isReadOnlyModel ? 'none' : 'block'}` }}>
                        <Icon type='plus' />
                        <Button
                          className='form-item-space'
                          type='link'
              ...
              <Card>
                <Card.Body title='限流规则'>
                  ...
                  <Form style={{ padding: '20px' }}>
                    ...
                    >
                      <Switch field={regex_combine_Field} disabled={isReadOnlyModel}></Switch>
                    </FormField>
                  </Form>
                    ...
                    {actionField.getValue() === LimitAction.UNIRATE && (
                      <FormField field={max_queue_delay_Field} label='最大排队时长'>
                        <InputAdornment after='秒'>
                          <InputNumber field={max_queue_delay_Field} min={1} hideButton disabled={isReadOnlyModel} />
                        </InputAdornment>
                      </FormField>
                    )}
                  </Form>
            ...
            <FormField label='是否启用' field={disableField}>
              <Switch field={disableField} disabled={isReadOnlyModel} />
            </FormField>
          </Form>
          <Form.Action>
            <Button type='primary' onClick={() => dispatch(creators.submit())} disabled={isReadOnlyModel}>
              提交
            </Button>
            <Button
              onClick={() => {
