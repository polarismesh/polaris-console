...
export default class BasicLayout extends React.Component<BasicPageProps, Readonly<{}>> {
  render() {
    ...
    const { type = 'page', title, operation, header, more, hideTitle, headerRender, full = true, onBack } = this.props
      ...
      <Layout>
        <Body>
          <Content>
            {!hideTitle &&
              (headerRender || (
                <Content.Header onBackButtonClick={onBack ? onBack : undefined} title={title} operation={operation}>
                  {header && header}
                </Content.Header>
              ))}
