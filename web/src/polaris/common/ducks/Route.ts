 ...
 * 路由功能
 */
import { BaseDuck, INIT, END, DuckOptions } from "saga-duck";
    ...
    this.params = params
      .map((param) => {
        return {
          ...
          defaults:
            typeof param.defaults === "function"
              ? param.defaults
              : () => param.defaults,
  ...
  stringify(routes) {
    const qs = this.params
      .map(({ key, route, stringify, defaults, alwaysVisible }) => {
        ...
        return (
          encodeURIComponent(route) + "=" + encodeURIComponent(stringify(value))
        );
      })
      .filter((v) => v)
      .join("&");
  ...
  *sagaToRouteWatch(duck = this) {
    ...
    yield takeLatest([INIT, types.SET, types.UPDATE], function* (action) {
      ...
      if (Object.keys(changes).length > 0) {
        ...
        yield call(
          perform,
          duck,
          duck.parser.stringify(curRoutes),
          duck.parser.shouldReplace(lastRoutes, curRoutes)
        );
      }
  ...
  protected watch({ options }: this): Channel<string> {
    return eventChannel((emitter) => {
      ...
      emitter(
        queryString ? window.location.hash : window.location.hash.split("?")[0]
      );
   ...
   * 实施URL变化
   */
  protected perform({ options }: this, params: string, replace: boolean) {
