...
export default abstract class GridDuck extends DuckMap {
  ...
  get reducers() {
    ...
    return {
      ...
      searchCondition: reduceFromPayload<string>(
        types.SET_SEARCH_CONDITION,
        ""
      ),
    };
  }
  ...
  get rawSelectors() {
    ...
    return {
      ...
      searchCondition: (state: State) =>
        state.searchCondition || (state.filter && state.filter.keyword),
  ...
  *sagaMain(duck = this) {
  ...
  /** 供外部完全掌握Grid的加载 */
  *sagaManualFetch(filter, searchDesc: string = "") {
  ...
  *sagaFetch() {
    ...
    try {
      // 加载数据
      let payload = yield getData(
        duck,
        filter,
        function* progressiveUpdate(payload) {
          yield put({
  ...
  abstract getData(
    me: this,
    filter,
    progressiveUpdate?: (result: GridResult) => any
  ): any;
