import { select, take, put, call, all, fork, cancel } from "redux-saga/effects";
  ...
  *sagaInitRoute(duck = this) {
    ...
    yield takeEvery([ducks.routes.types.CHANGE], function* routeToAttr(action) {
      ...
      const actions = Object.keys(routes).reduce(
        (actions, key) =>
          actions.concat(
            routeCreators[key] ? routeCreators[key](routes[key]) : []
          ),
        []
      );
      // 标记为路由向属性同步
    ...
    return (this._routesSelector = (state) =>
      this.params.reduce((map, param) => {
        ...
        if (param.selector) {
          ...
          routeSelector =
            selectors[param.key] || ((state) => selector(state)[param.key]);
        }
  ...
  get routeCreators() {
    ...
    return (this._routeCreators = this.params.reduce((map, param) => {
      ...
      if (param.creator) {
        ...
        creator =
          creators[param.key] || ((payload) => ({ type: param.type, payload }));
      }
