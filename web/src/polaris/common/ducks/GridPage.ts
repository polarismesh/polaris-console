...
import { Completer, UPDATE, combineCompleters, getBufferUpdate } from './utils/completer'
...
export default abstract class GridPageDuck extends PageDuck {
  ...
  *saga() {
  ...
  protected *sagaColumnsWatch() {
    ...
    yield runAndWatchLatest<this['ColumnOptions']>(this.columnsWatchTypes, selectors.columnOptions, function*(options) {
      // getColumns 有时候需要调后端接口, 加一个yield, @author seven
      yield put(creators.setFullColumns(yield duck.getColumns(duck, options)))
    })
  }
