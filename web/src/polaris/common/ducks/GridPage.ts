...
import { INIT, reduceFromPayload, createToPayload } from "saga-duck";
...
import {
  notification,
  TableColumn as TablePanelColumnGeneric,
} from "tea-component";
...
export default abstract class GridPageDuck extends PageDuck {
  ...
  get rawSelectors() {
    ...
    return {
      ...
      columnOptions: (state: State): this["ColumnOptions"] => ({}),
      /** 供FieldsManager使用 */
  ...
  *saga() {
  ...
  abstract getData(
    filter: this["GetDataParams"]
  ): Promise<GridResult<this["Item"]>>;
  /** getData入参的类型声明 */
  ...
  protected *sagaColumnsWatch() {
    const duck = this;
    const { types, creators, selector, selectors, ducks } = duck;
    yield runAndWatchLatest<this["ColumnOptions"]>(
      this.columnsWatchTypes,
      selectors.columnOptions,
      function* (options) {
        // getColumns 有时候需要调后端接口, 加一个yield, @author seven
        yield put(
          creators.setFullColumns(yield duck.getColumns(duck, options))
        );
      }
    );
  }
  ...
  protected *doExport(action) {
    ...
    try {
      ...
      do {
        ...
        yield fork(
          [duck, duck.sagaExportProgress],
          Math.round((100 * page) / totalPage)
        );
      ...
      do {
        ...
        const thisTry = lastTry
          ? Math.floor((lastTry - exported) / 1.5) + exported
          : fullList.length;
        const content = csvUtil.format(
          fullList.slice(exported, thisTry),
          columns
        );
        exceed = csvUtil.save(
          `${fileName}${tried ? "_" + (fileNum + 1) : ""}.csv`,
          content,
          false
        );
        if (exceed) {
