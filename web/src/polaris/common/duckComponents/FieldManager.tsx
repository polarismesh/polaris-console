...
import { purify, memorize, DuckCmpProps } from "saga-duck";
...
function selectionRemove<T extends Identifiable>(
  records: T[],
  selection: string[],
  item: T
): string[] {
 ...
 */
export default class FieldManagerButton extends React.Component<
  ButtonProps,
  any
> {
  ...
  componentWillReceiveProps(nexProps) {
    if (
      this.props.cacheKey !== nexProps.cacheKey ||
      this.props.fields !== nexProps.fields
    ) {
  ...
  render() {
    const { model } = this;
    const { style, fields, cacheKey, ...rest } = this.props;
    return (
      <React.Fragment>
        <Button
          style={style}
...
class CustomFields {
    ...
    if (selection) {
      ...
      selection = fields.reduce((selection, field) => {
        return field.required
          ? selectionInsert(fields, selection, field)
          : selection;
      }, selection);
    }
  ...
  getDisplayFields(selection = this.selection): ColumnField[] {
    let { fields = defaultValue.fields } = this;
...
interface MyDialogProps
  extends Omit<Props, "fields" | "cacheKey">,
    DuckCmpProps<DialogDuck> {
  model: CustomFields;
}
class FieldManager extends Component<MyDialogProps, State> {
  ...
  renderCol() {
    ...
    let { eachColNum = defaultValue.eachColNum } = this.props;
    const { fields = defaultValue.fields as ColumnField[] } = this.props.model;
    let { selection = [] } = this.state || {};
    let uls = [];
    fields.forEach((field, index) => {
      let required = !!field.required;
      let defaultShow = !field.hide;
      let indexUl = Math.floor(index / eachColNum);
      if (!uls[indexUl]) {
        uls[indexUl] = [];
      }
      const isChecked =
        required || selection.indexOf(field && String(field.id)) > -1;
      uls[indexUl].push(
        <li key={field.id}>
          <input
            type="checkbox"
            ...
            disabled={
              ...
              (!isChecked &&
                mandatoryMaxLimit &&
                count.selectedCount >= maxLimit)
            }
            checked={isChecked}
            onChange={(e) => {
              let checked = e.target.checked;
              let action = checked ? selectionInsert : selectionRemove;
              this._updateSelection(action(fields, selection, field));
            }}
          />
          <label htmlFor={`customize_field_${field.id}`}>
            {field.headTitle}
          </label>
        </li>
      );
        ...
        <Modal.Footer>
          <button
            ...
            onClick={(e) => {
              ...
              let fields = model.getDisplayFields(selection);
