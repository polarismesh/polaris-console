...
interface Addon {
  onInjectTip?: Middleware<
    (opts: SearchableListStatusTipProps) => React.ReactNode
  >;
}
/**
 * 列表
...
export function SearchableList<T>(props: SearchableListProps<T>) {
  ...
  return (
    <List onScrollBottom={nomore ? undefined : handlers.more} {...rest}>
      {!showEmpty && !isLoadMoreMode && tipProps && (
        <List.StatusTip>{statusTip}</List.StatusTip>
      )}
      {showEmpty && (
        <List.Item
...
function useMiddleware<K extends keyof Addon>(addons: Addon[], name: K) {
  ...
  return (initialValue: ValueType) =>
    addons
      ...
      .filter(Boolean)
      .reduce<ValueType>(
        (previous, middleware: Middleware<any>) => middleware(previous),
        initialValue
      );
}
