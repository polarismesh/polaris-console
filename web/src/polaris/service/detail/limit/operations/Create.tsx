...
import { Card, Form, FormItem, Segment, H3, Button, H4, Justify, MonacoEditor, Text, Switch } from 'tea-component'
import { LIMIT_RANGE_OPTIONS, LIMIT_TYPE_MAP, LIMIT_TYPE_OPTIONS, LIMIT_THRESHOLD_OPTIONS } from '../types'
...
const renderLimitRule = props => {
  ...
  return (
    <>
      ...
      <Form style={{ width: '850px' }}>
        <Form style={{ width: '100%' }} layout='inline'>
          <FormField
            field={methodValue}
            label={'接口'}
            message={methodType.getValue() === MATCH_TYPE.REGEX && REGEX_STAR_TIPS}
          >
      ...
      <FormItem label={<H3 style={{ margin: '10px 0' }}>如果满足以下任意条件，进行限流</H3>}></FormItem>
      <Form>
        <Form style={{ width: '850px' }}>
          {[...amounts.asArray()].map((field, index) => {
            const { maxAmount, validDuration } = field.getFields(['maxAmount', 'validDuration'])

            return (
              <Form layout={'inline'}>
                <FormField field={validDuration} label={'统计时长'}>
                  <InputNumber hideButton min={0} unit={'秒'} field={validDuration} />
                </FormField>
                <FormField field={maxAmount} label={'请求数'}>
                  <InputNumber hideButton min={0} unit={'次'} field={maxAmount} />
                </FormField>
                {amounts.getValue()?.length > 1 && (
                  <>
                    <Button type='icon' icon='close' onClick={() => removeArrayFieldValue(amounts, index)}></Button>
                  </>
                )}
                <Button type={'icon'} icon={'plus'} onClick={() => addAmount(amounts)}></Button>
              </Form>
            )
          })}
        </Form>
...
export default purify(function CreateRoute(props: DuckCmpProps<CreateRouteDuck>) {
  ...
  const formApi = form.getAPI(store, dispatch)
  const { editType, type, jsonValue } = formApi.getFields(['editType', 'type', 'jsonValue'])
    ...
    <>
      <Form>
        ...
        {editType.getValue() === EditType.Json && (
          <FormItem
            message={<Text theme={'danger'}>{jsonValue.getTouched() && jsonValue.getError()}</Text>}
            label={'JSON编辑'}
          >
            <section style={{ border: '1px solid #cfd5de', width: '1000px' }}>
              <MonacoEditor
                ref={ref}
                monaco={monaco}
                height={400}
