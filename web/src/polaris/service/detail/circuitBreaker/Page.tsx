          ...
          left={
            <>
              ...
              <Button
                ...
                disabled={isReadOnly(namespace) || drawerStatus.visible || !edited}
                tooltip={isReadOnly(namespace) ? "该命名空间为只读的" : !edited ? "未更改" : "向服务器端提交变更"}
                style={{ marginTop: "20px" }}
              >
                提交
              </Button>
                              ...
                              <FormText>
                                {Object.keys(destination.policy).map((key, index) => {
                                  ...
                                  if (key === PolicyName.ErrorRate) {
                                    return (
                                      <Text parent="p" key={index}>{`当请求个数大于${
                                        destination.policy[key]?.requestVolumeThreshold || 10
                                  ...
                                  if (key === PolicyName.SlowRate) {
                                    return (
                                      <Text parent="p" key={index}>{`以超过${
                                        destination.policy[key]?.maxRt ?? "-"
                                      }的请求作为超时请求，${PolicyMap[key]?.text ?? "-"}大于${
                                        destination.policy[key]?.slowRateToOpen ?? "-"
                                      }%时熔断`}</Text>
                                    );
                                  }
                                  if (key === PolicyName.ConsecutiveError) {
                                    return (
                                      <Text parent="p" key={index}>{`当连续请求错误超过${
                                        destination.policy[key]?.consecutiveErrorToOpen ?? "-"
                                      }个时熔断`}</Text>
                                    );
                                  }
                                })}
