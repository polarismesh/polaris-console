...
function delDir(dest) {
  ...
  paths.forEach(function (p) {
    ...
    if (st.isDirectory()) {
      console.log(
        `\r${styles.red[0]}Delete Directory${styles.red[1]}: ${target}`
      );
      delDir(target);
    }
  });
...
function copyDir(source, dest) {
  ...
  paths.forEach(function (p) {
    ...
    if (st.isFile()) {
      ...
      console.log(
        `\r${styles.yellow[0]}Copy File${styles.yellow[1]}: ${target}`
      );
    ...
    if (st.isDirectory()) {
      ...
      console.log(
        `\r${styles.yellow[0]}Create Directory${styles.yellow[1]}: ${target}`
      );
      fs.mkdirSync(target);
      copyDir(src, target);
    }
